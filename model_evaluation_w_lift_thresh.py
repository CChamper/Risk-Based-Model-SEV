...
def lift_curve(y_true, p_pred, ks=None):
    """Compute lift over a range of k values (fractions)."""
    if ks is None:
        ks = np.linspace(0.01, 0.2, 20)
    lifts = []
    for k in ks:
        lift, _, _ = lift_at_k(y_true, p_pred, k=k)
        lifts.append(lift)
    return ks, np.array(lifts)
...
def per_signal_lift(df, y_true, p_pred, k=0.05, min_count=100):
    """
    Compute lift@k per signal with a minimum sample threshold.
    Returns a DataFrame with signal, n, lift, topk_rate, overall_rate.
    """
    y_true = np.asarray(y_true)
    p_pred = np.asarray(p_pred)
    out = []
    for sig, idx in df.groupby("signal").indices.items():
        idx = np.asarray(idx)
        if len(idx) < min_count:
            continue
        lift, topk_rate, overall_rate = lift_at_k(y_true[idx], p_pred[idx], k=k)
        out.append({
            "signal": sig,
            "n": len(idx),
            "lift": lift,
